...
async def analyst_journey(page: Page) -> None:
    # CytoPedia search + category filter
    cytopedia_url = f"{settings.base_url.rstrip('/')}/cytopedia"
    search = page.get_by_role(
        "textbox", name="Search terms by title or description"
    )
    try:
        await _goto_and_wait(page, cytopedia_url, search)
    except PlaywrightTimeoutError:
        return
